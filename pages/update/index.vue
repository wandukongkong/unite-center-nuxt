...
const { motionProperties: detailAreaMotionProperties } = useMotionProperties(detailArea, {
	opacity: 0,
	x: -500,
});
...
const setGrouppedPokemonList = () => {
	// 날짜 별로 Group
	const groupedPokemonInfo = unitePokemonList.value.reduce((result, unitePokemonInfo) => {
		const lastUpdatedDate = unitePokemonInfo.updatedList
			.sort((a, b) => {
				const aUpdatedDateNumber = Number(dayjs(a.updatedDate, 'YYYY-MM-DD').format('YYYYMMDD'));
				const bUpdatedDateNumber = Number(dayjs(b.updatedDate, 'YYYY-MM-DD').format('YYYYMMDD'));
	...
	groupedPokemonList.value = Object.keys(groupedPokemonInfo)
		.sort((a, b) =>
			Number(dayjs(a, 'YYYY-MM-DD').format('YYYYMMDD')) < Number(dayjs(b, 'YYYY-MM-DD').format('YYYYMMDD')) ? 1 : -1,
		)
		.map(key => ({
			updatedDate: key,
			pokemonList: [],
...
<template>
	<div ref="container">
      ...
      <UInput class="w-[100%] mb-2 opacity-0"></UInput>
			...
			<!-- 리스트 영역 -->
			<Transition>
				<div class="reative flex flex-col justify-start py-3 bg-transparent" :class="isMobile ? 'mt-10' : ''">
					<Transition-group>
						<div
						...
						>
							...
							<div class="flex flex-wrap mb-5">
								<TransitionGroup name="bounce">
									<div
										v-for="(pokemonInfo, pokemonListIndex) in groupedPokemonInfo.pokemonList"
										...
										>
											<NuxtImg
												v-if="
													...
													(pokemonInfo.updatedList[0]?.['updateStates'] || []).includes('new')
												"
											...
											<div
												v-if="
													!(
														pokemonInfo.updatedList.find(
															updateInfo => updateInfo.updatedDate === groupedPokemonInfo.updatedDate,
														)?.updateStates || []
													).includes('new')
												"
